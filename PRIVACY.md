...
- When you sign in to **OVA MRS** or **CAC Locker** inside the app, your credentials go directly to those services through an embedded web view. **Bior never sees, captures, or stores your username or password.**
...
The app stores the following on your device. Nothing in this list ever leaves the device under Bior's control:
...
- **Connection flags** — booleans noting whether you've previously signed in to MRS or CAC (so we can show the right "Connected" badge). The actual session is held by the embedded web view's own cookie store, not by Bior.
...
- You sign in directly with the service. Bior **does not capture, log, or transmit** your username, password, or session cookie. The cookie is held by the platform's web view component and scoped to that site only.
- Bior injects **display-only CSS** to make the desktop layouts more readable on a phone. Bior does not modify, extract, or repost content from those sites.
- The "Disconnect" button inside Bior clears the connection flag described above. To fully sign out, also tap the service's own Sign Out before disconnecting.

### Public tournament data (AES, Timu)

The app fetches **public tournament results and metadata** from `results.advancedeventsystems.com` and Timu Sports. This is the same data anyone can see by visiting those sites in a browser. The fetched data is cached on your device so the app works on weak connections; it is not transmitted to any Bior server.

### Crash and error reporting (Sentry)

Production builds of Bior include **Sentry** for crash reporting. When the app crashes or hits an unhandled error, Sentry records:
...
Bior surfaces public tournament data for youth volleyball events, but does not knowingly collect data from anyone, including children. There is no account, no signup, and no on-device data is transmitted off-device under our control.
...
- We do not maintain backups of your device data on any Bior-controlled server (there is no such server).

## Your rights

Because we hold no personal data on any server, the standard "delete my data" request resolves to "uninstall the app".

For data Bior receives via Sentry (anonymized crash traces), you may contact us to request deletion: **jontycollis@gmail.com**.
